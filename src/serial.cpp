...
static void write_be16(std::vector<uint8_t>& v, uint16_t x) { v.push_back((x>>8)&0xFF); v.push_back(x&0xFF); }
...
static void send_response(uint8_t flag, const uint8_t* payload, size_t payload_len) {
    ...
    body.push_back(flag);
    if (payload && payload_len) body.insert(body.end(), payload, payload + payload_len);
...
static std::vector<uint8_t> get_realtime_payload() {
    ...
    v.push_back(0x12); v.push_back(0x34); // RPM = 0x1234 (4660)
    v.push_back(0x00); v.push_back(0x10); // seconds = 16
    v.push_back(0x00); v.push_back(0x01); // dummy
    return v;
}
...
static bool write_table(uint8_t canid, uint16_t table, uint16_t offset, const uint8_t* data, uint16_t size) {
    ...
    (void)canid; (void)table; (void)offset; (void)data; (void)size;
...
static void get_f_info(uint8_t canid, std::vector<uint8_t>& out) {
    ...
    out.push_back(0x08); out.push_back(0x00); // 2048 -> 0x0800
    out.push_back(0x08); out.push_back(0x00); // write blocking factor 2048
}
...
static void handle_command(const uint8_t* payload, size_t payload_len) {
    ...
    switch (cmd) {
        ...
        case 'r': {
            ...
            if (payload_len < 1 + 1 + 1 + 2 + 2) { // command + CANid + Table + Offset(2) + Size(2)
                send_response(0x83, nullptr, 0);
                break;
            }
            uint8_t canid = payload[1];
...
static void process_incoming_stream() {
    ...
    while (true) {
        if (intake_buf.size() < 2) return; // need size
