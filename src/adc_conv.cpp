...
void ego_update(GlobalState* gs, uint16_t raw_adc)
{
    ...
    gs->air_fuel_ratio = raw_adc * (2239 - 735) / 0x3FFF + 735;
}
...
void bat_update(GlobalState* gs, uint16_t raw_adc)
{
    ...
    // 0V =  0.7V = 0x0000 ADC
    // 5V = 30.7V = 0x3FFF ADC
    gs->battery_voltage = raw_adc * 3000 / 0x3FFF + 70;
}
...
void map_update(GlobalState* gs, uint16_t raw_adc)
{
    ...
    gs->manifold_pressure = raw_adc * 2500 / 0x3FFF + 100;
}
...
void tps_update(GlobalState* gs, uint16_t raw_adc)
{
    ...
    if (raw_adc < 0x0666)
        ...
        gs->throttle_position = (raw_adc - 0x0666) * 1000 / (0x3999 - 0x0666);
}